...
class Alumni_excel extends CI_Model
{
    ...
    public function export(){
        ...
        ->setSubject('Data Alumni')
        ->setDescription('Data Alumni Tahun '.date("Y"))
        ->setKeywords('Alumni')
        ->setCategory('Alumni');

        $spreadsheet->setActiveSheetIndex(0)->setCellValue('A3', 'Data Alumni '.date("Y"));
        ...
        ->setCellValue('H'.$startRowHeader, 'Tahun Lulus')
        ->setCellValue('I'.$startRowHeader, 'Email')
        ->setCellValue('J'.$startRowHeader, 'Hp')
        ->setCellValue('K'.$startRowHeader, 'Alamat')
        ;
        ...
        $index = 0;
        $data_siswa = $this->Alumni_model->get_all();
        foreach ($data_siswa as $key => $siswa) {
            ...
            ->setCellValue('H'.$startRowBody, $siswa->tahun_lulus)
            ->setCellValue('I'.$startRowBody, $siswa->email)
            ->setCellValue('J'.$startRowBody, $siswa->hp)
            ->setCellValue('K'.$startRowBody, $siswa->alamat)
            ;
            $startRowBody++;
        };

        $spreadsheet->getActiveSheet()->setTitle('Data Alumni '.date('Y'));
        $spreadsheet->setActiveSheetIndex(0);
        ...
        for ($row = 1; $row <= $highestRow; $row++){                  //  Read a row of data into an array                 
            ...
            if ($startImport==TRUE) {
                $res[] = array(
                    ...
                    "tahun_lulus" => $cellData[0][$startColumnIndex + 6],
                    "email"=> $cellData[0][$startColumnIndex+7],
                    "hp"=> $cellData[0][$startColumnIndex+8],
                    "alamat"=> $cellData[0][$startColumnIndex+9]
                );
            }else{
                if (
                    ...
                    strtolower($cellData[0][7]) == "tahun lulus" &&
                    strtolower($cellData[0][8]) == "email" &&
                    strtolower($cellData[0][9]) == "hp"
                ) {
                    $startImport = TRUE;
                    $startColumnIndex = 1;
                }elseif (
                    ...
                    strtolower($cellData[0][6]) == "tahun lulus" &&
